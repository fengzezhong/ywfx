...
exports.pagingQuery = function(modelInst, page, size, conditionMap, tableType, cb, populate, sort) {
    ...
    query.exec(function(err,rs){
        if(err){
            ...
            //计算数据总数
            modelInst.find(conditionMap, function(err,result){
                if(err){
                    cb(exports.returnMsg(false, '1001', '分页查询时出现异常。', null, err));
                }else {
                    if(tableType == 'easyui') {
                        cb(exports.returnMsg4EasyuiPaging(true, '0000', '分页查询成功。', rs, result.length));
                    }
                    else if(tableType == 'datatables') {
                        cb(exports.returnMsg4Paging(true, '0000', '分页查询成功。', rs, result.length));
                    }
