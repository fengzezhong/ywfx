...
// helper to migu that a value is the same as some target, within our tolerance
...
/*
	migu(renderMode + ' jsPlumb.removeAllEndpoints after element removed from DOM', function() {
...
// **************************** DETACHING CONNECTIONS ****************************************************
    ...
    // migu that detach does not fire an event by default
	test(renderMode + ': _jsPlumb.detach should fire detach event by default', function() {
		...
		var eventCount = 0;
		_jsPlumb.bind("connectionDetached", function(c) { eventCount++; });
    ...
    // migu that detach fires an event when instructed to do so
	...
	//TODO make sure you run this migu with a single detach call, to ensure that
...
*/
	...
	/*
    migu(renderMode + ": removeAllEndpoints - element already deleted", function() {
        var d1 = _addDiv("d1");
        _jsPlumb.addEndpoint(d1);
        _jsPlumb.addEndpoint(d1);
        _jsPlumb.addEndpoint(d1);
...
/*
	...
	migu does.  But you can of course get there accidentally, which is one reason why it would be good
	for this migu to exist.

    migu(renderMode + ": deleting endpoints of deleted element should not fail", function() {
        var d1 = _addDiv("d1"), d2 = _addDiv("d2");
...
// jsPlumb.connect, after makeSource has been called on some element
    ...
    What i would *like* this migu to do is to fake the user having dragged a connection from
    d16 to d17.  the mousedown on d16 is recognised, and an endpoint is added. but the rest of it
    is not.  so the migu fails by saying that there's 1 endpoint on d16 when i expected none, and
    ...
    migu(renderMode + ": _jsPlumb.makeTarget - endpoints deleted when detached on callback.", function() {
		var d16 = _addDiv("d16"), d17 = _addDiv("d17"); 
		_jsPlumb.makeSource(d16);
		_jsPlumb.makeTarget(d17);
        var detached = false;
...
// *********************** end of makeTarget (and associated methods) tests ************************ 
	...
	test(renderMode + ": _jsPlumb.connect (Connector migu, straight)", function() {
		var d16 = _addDiv("d16"), d17 = _addDiv("d17"); 
	...
	test(renderMode + ": _jsPlumb.connect (Connector migu, bezier, no params)", function() {
		var d16 = _addDiv("d16"), d17 = _addDiv("d17"); 
	...
	test(renderMode + ": _jsPlumb.connect (Connector migu, bezier, curviness as int)", function() {
		var d16 = _addDiv("d16"), d17 = _addDiv("d17"); 
	...
	test(renderMode + ": _jsPlumb.connect (Connector migu, bezier, curviness as named option)", function() {
		var d16 = _addDiv("d16"), d17 = _addDiv("d17"); 
	...
	test(renderMode + ": _jsPlumb.connect (Connector as string migu)", function() {
		var d16 = _addDiv("d16"), d17 = _addDiv("d17"); 
		var conn = _jsPlumb.connect({ source:d16, target:d17, connector:"Straight" });
		assertContextSize(3);
		assertConnectionByScopeCount(_jsPlumb.getDefaultScope(), 1, _jsPlumb);
	...
	test(renderMode + ": _jsPlumb.connect (Endpoint migu)", function() {
		var d16 = _addDiv("d16"), d17 = _addDiv("d17"); 
	...
	test(renderMode + ": _jsPlumb.connect (Endpoint as string migu)", function() {
		var d16 = _addDiv("d16"), d17 = _addDiv("d17"); 
		var conn = _jsPlumb.connect({ source:d16, target:d17, endpoint:"Rectangle" });
		assertContextSize(3);
		assertConnectionByScopeCount(_jsPlumb.getDefaultScope(), 1, _jsPlumb);
	...
	test(renderMode + ": _jsPlumb.connect (Endpoints migu)", function() {
		var d16 = _addDiv("d16"), d17 = _addDiv("d17"); 
	...
	test(renderMode + ": _jsPlumb.connect (by Endpoints, connector migu)", function() {
		var d16 = _addDiv("d16"), d17 = _addDiv("d17"); 
	...
	test(renderMode + ": _jsPlumb.connect (by Endpoints, connector as string migu)", function() {
		var d16 = _addDiv("d16"), d17 = _addDiv("d17");
		var e16 = _jsPlumb.addEndpoint(d16, {});
		var e17 = _jsPlumb.addEndpoint(d17, {});
		var conn = _jsPlumb.connect({ sourceEndpoint:e16, targetEndpoint:e17, connector:"Straight" });
	...
	test(renderMode + ": _jsPlumb.connect (by Endpoints, anchors as string migu)", function() {
		var d16 = _addDiv("d16"), d17 = _addDiv("d17");
	...
	test(renderMode + ": _jsPlumb.connect (connect by element, default endpoint, supplied dynamic anchors)", function() {
		...
		assertEndpointCount("d2", 1, _jsPlumb);
		_jsPlumb.detach({source:d1, target:d2});
		// this changed in 1.3.5, because auto generated endpoints are now removed by detach.  so i added the migu below this one
	...
	test(renderMode + ": _jsPlumb.connect (connect by element, default endpoint, supplied dynamic anchors, delete on detach false)", function() {
		...
		_jsPlumb.detach({source:d1, target:d2});
		// this changed in 1.3.5, because auto generated endpoints are now removed by detach.  so i added this migu
		// to check that the deleteEndpointsOnDetach flag is honoured.
		assertEndpointCount("d1", 1, _jsPlumb);assertEndpointCount("d2", 1, _jsPlumb);
		assertContextSize(2);
	});
	...
	test(renderMode + ": _jsPlumb.connect (connect by element, supplied endpoints using 'source' and 'target' (this migu is identical to the one above apart from the param names), and dynamic anchors)", function() {
		var d1 = _addDiv("d1"), d2 = _addDiv("d2"), d3 = _addDiv("d3");
		var endpoint = { isSource:true };
		var e1 = _jsPlumb.addEndpoint(d1, endpoint);
		var e2 = _jsPlumb.addEndpoint(d2, endpoint);
	...
	// this migu is for the original detach function; it should stay working after i mess with it
	// a little.
...
/*
    migu(renderMode + ": _jsPlumb.detach (params object, using target only)", function() {
		var d1 = _addDiv("d1"), d2 = _addDiv("d2"), d3 = _addDiv("d3"),
...
*/
	...
	test(renderMode + ": Endpoint.isVisible/setVisible basic migu (no connections)", function() {
		var d1 = _addDiv("d1"), d2 = _addDiv("d2");
		var e1 = _jsPlumb.addEndpoint(d1);
	...
	test(renderMode + " _jsPlumb.Defaults.Container, specified with DOM element", function() {		
		...
		// migu to see if 3 elements have been added
	...
	/*
	migu(renderMode + " _jsPlumb.hide, two-arg version, endpoints should also be hidden", function() {
		var d1 = _addDiv("d1"), d2 = _addDiv("d2"),
		e = { isSource:true, isTarget:true, maxConnections:-1 },
		e1 = _jsPlumb.addEndpoint(d1, e),
		e2 = _jsPlumb.addEndpoint(d2, e),
	...
	/**
	 * migu for issue 132: label leaves its element in the DOM after it has been 
...
// setId function
	...
	test(renderMode + " select, basic migu", function() {
		_addDiv("d1"); _addDiv("d2");
		var c = _jsPlumb.connect({source:"d1", target:"d2"}),
	...
	test(renderMode + " select, basic migu with multiple scopes; dont filter on scope.", function() {
		_addDiv("d1"); _addDiv("d2");
	...
	test(renderMode + " select, basic migu with multiple scopes; filter on scope", function() {
		_addDiv("d1"); _addDiv("d2");
		var c = _jsPlumb.connect({source:"d1", target:"d2", scope:"FOO"}),
			c2 = _jsPlumb.connect({source:"d1", target:"d2", scope:"BAR"}),
	...
	test(renderMode + " select, basic migu with multiple scopes; filter on scopes", function() {
		_addDiv("d1"); _addDiv("d2");
		var c = _jsPlumb.connect({source:"d1", target:"d2", scope:"FOO"}),
			c2 = _jsPlumb.connect({source:"d1", target:"d2", scope:"BAR"}),
	...
	test(renderMode + " select, basic migu with multiple scopes; scope but no scope filter; single source id", function() {
		_addDiv("d1"); _addDiv("d2");
		var c = _jsPlumb.connect({source:"d1", target:"d2", scope:"FOO"}),
			c2 = _jsPlumb.connect({source:"d1", target:"d2", scope:"BAR"}),
			c3 = _jsPlumb.connect({source:"d1", target:"d2", scope:"BAZ"}),
	...
	test(renderMode + " select, basic migu with multiple scopes; filter on scopes; single source id", function() {
		_addDiv("d1"); _addDiv("d2");
		var c = _jsPlumb.connect({source:"d1", target:"d2", scope:"FOO"}),
			c2 = _jsPlumb.connect({source:"d1", target:"d2", scope:"BAR"}),
			c3 = _jsPlumb.connect({source:"d1", target:"d2", scope:"BAZ"}),
	...
	test(renderMode + " select, basic migu with multiple scopes; filter on scope; dont supply sourceid", function() {
		_addDiv("d1"); _addDiv("d2");
		var c = _jsPlumb.connect({source:"d1", target:"d2", scope:"FOO"}),
			c2 = _jsPlumb.connect({source:"d1", target:"d2", scope:"BAR"}),
...
// setPaintStyle/getPaintStyle tests
	...
	*
	migu(renderMode + " clone paint style", function() {
		var d1 = _addDiv("d1"), d2 = _addDiv("d2"), d3 = _addDiv("d3"),
...
// ******************  connection type tests - types, type extension, set types, get types etc. *****************
	...
	test(renderMode + " setType, scope", function() {
		...
		_jsPlumb.Defaults.ConnectionsDetachable = true;//just make sure we've setup the migu correctly.
...
// ------------- utility functions - math stuff, mostly --------------------------
	...
	test(renderMode + "jsPlumbUtil typeof functions", function() {
		for (var i = 0; i < types.length; i++) {
			...
			// first, migu that the object type is identified correctly
			equal(f(v), true, types[i].t + " is recognised as " + types[i].t);
			// now migu that everything else is recognised as not being of this type
	...
	/*
	 * migu the merge function in jsplumb util: it should create an entirely new object
	 */
