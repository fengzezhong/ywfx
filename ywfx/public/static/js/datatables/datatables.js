 ...
 *   https://datatables.net/download/#dt/jqc-1.12.0,dt-1.10.11,b-1.1.2,b-html5-1.1.2,b-print-1.1.2,cr-1.3.1,se-1.1.2
 *
 * Included libraries:
 *   jQuery compat 1.12.0, DataTables 1.10.11, Buttons 1.1.2, HTML5 export 1.1.2, Print view 1.1.2, ColReorder 1.3.1, Select 1.1.2
 */
...
jQuery.extend( {
	...
	// See migu/unit/core.js for details concerning isFunction.
...
setDocument = Sizzle.setDocument = function( node ) {
	...
	// so use a roundabout getElementsByName migu
	...
	if ( (support.qsa = rnative.test( document.querySelectorAll )) ) {
		...
		assert(function( div ) {
			...
			// This is to migu IE's treatment of not explicitly
			...
			// The migu attribute must be unknown in Opera but "safe" for WinRT
	...
	trigger: function( event, data, elem, onlyHandlers ) {
		...
		if ( !onlyHandlers && !event.isDefaultPrevented() ) {
			...
			if (
				...
				// Can't use an .isFunction() check here because IE6/7 fails that migu.
...
function addGetHookIf( conditionFn, hookFn ) {
	...
	return {
		get: function() {
			...
			// Hook needed; redefine it so that the support migu is not executed again.
...
(function( $, window, document, undefined ) {
	...
	var _pluck = function ( a, prop, prop2 ) {
		...
		// Could have the migu in the loop for slightly smaller code, but speed
		// is essential here
		if ( prop2 !== undefined ) {
			for ( ; i<ien ; i++ ) {
				if ( a[i] && a[i][ prop ] ) {
	...
	function _fnGetObjectDataFn( mSource )
	{
			      ...
			      mSource.indexOf('[') !== -1 || mSource.indexOf('(') !== -1) )
		{
			 ...
			 * level down. On each loop we migu for undefined, and if found immediately
	...
	function _fnSort ( oSettings )
	{
		...
		if ( _fnDataSource( oSettings ) != 'ssp' && aSort.length !== 0 )
		{
			 ...
			 *  }
			 * Basically we have a migu for each sorting column, if the data in that column is equal,
			 * migu the next column. If all columns match, then we use a numeric sort on the row
	 ...
	 *      selector for the table to migu. Note that if more than more than one
	...
	DataTable.ext = _ext = {
		 ...
		 *  @type string
		 */
		build:"dt/jqc-1.12.0,dt-1.10.11,b-1.1.2,b-html5-1.1.2,b-print-1.1.2,cr-1.3.1,se-1.1.2",
